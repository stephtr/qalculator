...
import { type KeyboardEvent, use, useEffect, useState } from "react";
...
export default function CalculationSheetPage() {
	...
	useEffect(() => {
		...
		if (cellsArray.length === 0) {
			cellsArray.push([
				{ id: Math.random().toString(), input: "", output: "" },
			]);
		}
				...
				return (
					<div key={cell.id} className="mt-2">
						<input
							...
							value={cell.input}
							onChange={(e) =>
								handleInputChange(cell.id, e.currentTarget.value)
							}
							onKeyUp={(e) => handleKeyUp(cell.id, e)}
						/>{" "}
						={" "}
						{/* biome-ignore lint/security/noDangerouslySetInnerHtml: <explanation> */}
						<span dangerouslySetInnerHTML={{ __html: cell.output }} />
					</div>
				);
			})}
		</div>
